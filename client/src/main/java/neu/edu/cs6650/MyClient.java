...
//    timer.scheduleAtFixedRate(new PeriodCount(request, timestamp, list), 0, 1000);
    ...
    for(int i = 0; i < phases.length; i++) {
      System.out.println(phases[i] + " phase:");
      ...
      System.out.println(phases[i] + " phase complete: " + dur +" s");
    }
    System.out.println("Wall Time " + String.format("%.01f", walltime) +" s");
    Collections.sort(measure.latency);
    System.out.println("95: " + measure.latency.get((int)(measure.latency.size() * 0.95)));
    System.out.println("99: " + measure.latency.get((int)(measure.latency.size() * 0.99)));
    System.out.println("Total requests: " +measure.requests);
    GenerateOutput output = new GenerateOutput(measure.throughput, "output256lb.csv");
    output.writeCsvFile();
...
//https://s3-us-west-2.amazonaws.com/linzishengec2bucket/server-0.0.1-SNAPSHOT.jar
//ssh -i "MyKeyPair.pem" ec2-user@ec2-54-191-173-130.us-west-2.compute.amazonaws.com
  public static void main(String[] args) throws InterruptedException {
    if(args.length < 5) {
      args = new String[5];
      args[0] = "256";
      args[1] = "35.197.4.76:8080/";
      //args[1] = "http://ec2-34-216-76-52.us-west-2.compute.amazonaws.com:8080/";
      args[2] = "1";
      args[3] = "10000";
      args[4] = "80";
    }
